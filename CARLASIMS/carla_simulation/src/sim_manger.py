...
class SimManager:
    def __init__(self, cond_ini, servers):
        ...
        self.sim_register = {
            ...
            'actorList': copy.deepcopy(cond_ini['actorList']),
        }
    ...
    def world_tick(self, save_actors, real_time):#, show):
        ...
        self.isbrake.append(self.world.get_actor(ego_id).get_control().brake)
        if len(self.isbrake) > 15:
            self.isbrake.pop(0)
            self.sim_register["brake"] |= sum(self.isbrake) / len(self.isbrake) > 0.68
        if save_actors:
    ...
    def __load_actors(self):
        for actor in self.sim_register["actorList"]:
            ...
            # print(spawn_point)
            # carla_actors = []
            new_actor = self.world.try_spawn_actor(bp, spawn_point)
    ...
    def __person_control(self, person_pose):
        ...
        if len(person_pose) > 1:
            pos_ant = np.array(person_pose[-2][:2])
            velocity = (pos - pos_ant)
            current_speed = np.linalg.norm(velocity) / self.revision_time
            if current_speed > 0.1:
                direction = np.rad2deg(np.arctan2(velocity[1], velocity[0]))
                # print("Dirección", direction, current_speed)
                speed = current_speed
                desv_direct = 10
            else:
                speed *= 0.4
    ...
    def __vehicle_control(self, vehicle_pose):
        pos = np.array(vehicle_pose[-1][:2])
        current_velocity = self.max_velocity
        if len(vehicle_pose) > 1:
            pos_ant = np.array(vehicle_pose[-2][:2])
