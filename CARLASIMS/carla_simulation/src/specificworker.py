...
def carla_fun():
    ports = [2000, 4000, 6000]#, 15000, 20000, 40000, 60000, 63000]
...
class SpecificWorker(GenericWorker):
    ...
    def compute(self):
        ...
        if self.cond_simulation is not None:
            ...
            @storeInQueue
            def multitask(cond_sim, servers, revision_time, fixed_delta, name, n_sim):
                # print(name)
                cpu_usage = []
                compute_time = []
                j = 0
                ...
                while j < n_sim:
                    real_time = 0.0
                    time_ini1 = time.time()
                    s = SimManager(cond_sim, servers)
                    #print("time initttt", time.time()-time_ini1)
                    i = 1
                    save_actor = False
                    # k = 0
                    while real_time < cond_sim['duration']:
                        time_ini = time.time()
            ...
            for i in self.servers:
                t1 = threading.Thread(name=f'sim{c}', target=multitask, args=(
                self.cond_simulation, i, self.revision_time, self.fixed_delta_seconds, f'sim{c}', 5))
                t1.start()
                c += 1
    ...
    def Carla_getState(self):
        ...
        if self.results['valid']:
            ...
            for r in self.results['fullResult']:
                print(r['collision']['actorcollision'])
                result = ifaces.RoboCompCarla.Simresult()
                result.indice = r['indice']
