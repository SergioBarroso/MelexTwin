...
//    if ((robot_nose-vtp).norm()< consts.final_distance_to_target) { //coppelia robot
        ...
        cout << "ROBOT STOPED BECAUSE COLLISION COULD HAPEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE----------------" << endl;
        check_task_att( true);
        return std::make_tuple(0.0, 0.0, 0.0);
    ...
    if (auto task = G->get_node(task_name); task.has_value())
    {
        ...
        else if (auto task_dest = G->get_attrib_by_name<task_destination_values_att>(task.value()); task_dest.has_value() and task_dest.value().get()[0] == task_point.x())
        {
            if(auto on_movement = G->get_attrib_by_name<task_movement_att>(task.value()); on_movement.has_value() and on_movement==false) {
                ...
                cout << "---------------------------------------------------------------UPDATE_NODE_TASK" + task_name + "IJOPUTA " << endl;
            }
        }
        G->update_node(task.value());
//        cout << "---------------------------------------------------------------UPDATE_NODE_TASK" +task_name"+ IJOPUTA " << endl;

//        if (task_point.x() != (G->get_attrib_by_name<task_x_value_att>(task.value())).value() or task_point.y() != (G->get_attrib_by_name<task_y_value_att>(task.value())).value()){
//            task_point.x() = (G->get_attrib_by_name<task_x_value_att>(task.value())).value();
//            cout << "PUNTO TAREA X" <<  task_point.x() << endl;
//            task_point.y()= (G->get_attrib_by_name<task_y_value_att>(task.value())).value();
//            cout << "PUNTO TAREA Y" <<  task_point.y() << endl;
//        }
