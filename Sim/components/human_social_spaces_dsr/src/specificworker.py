...
class SpecificWorker(GenericWorker):
    ...
    def get_people_from_dsr(self):
        ...
        for person_node in people_nodes:
            ...
            # print("TX",tx,ty,tz)
            # print("RX",rx,ry,rz)
            person_type = PersonType(person_id, tx, ty, rz)
    ...
    def get_interactive_objects_from_drs(self):
        ...
        for edge in interactive_edges:
            ...
            object_type = ObjectType(obj_id, object_node.id, tx, ty, rz, shape, inter_angle, inter_space, depth, width,
                                     height)
            ...
            if person_node_in_dsr is not None:
                ...
                try:
                    self.rt_api.insert_or_assign_edge_RT(self.g.get_node('world'), person_node_in_dsr.id,
                                                         [person.x, person.y, person.z], [.0, .0, person.ry])
                except:
                    traceback.print_exc()
